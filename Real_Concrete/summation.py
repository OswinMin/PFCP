...
def draw(ReList:list[pd.DataFrame], title, auxInd, savepath):
    ...
    axes[1].set_ylim((min(np.min(FCP_size), np.max(FCP_size) / 4 * 3), axes[1].get_ylim()[1]))
...
if __name__ == '__main__':
    checkDir("../Figure")
    checkDir("../Figure/Concrete")
    for alpha in [0.1]:
        n, Y_int, tar_ind, sepN, seed = 36, 10, 3, 10, 1
        auxInd = [1,2,3,4,5,6,7]
        RpathList = [f"../Result/Real_Concrete/{n}_{auxN}_{tar_ind}_{sepN}_{seed}_{alpha}" for auxN in auxInd]
        nameList = ["LCP", "FCP", "FedCP", "FedCP-QQ", "CPlab", "CPhet"]
        ReList, _ = summation_matrix(RpathList, nameList, alpha)
        title = f'CONCRETE ($n={n},\\alpha={alpha}$)'
        savepath = f"../Figure/Concrete/nagent_{n}_{alpha}.pdf"
        draw(ReList, title, auxInd, savepath)

    Rpath = "../Result/Real_Concrete/50_5_3_10_1_"
    nameList = ["LCP", "FCP", "FedCP", "FedCP-QQ"]
    alphaList = [0.1]
    LoccovList, LoccovStdList = summation_loc([Rpath+f"{a}" for a in alphaList], nameList)
    savepath = f"../Figure/Concrete/conditional.pdf"
