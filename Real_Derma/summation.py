...
if __name__ == '__main__':
    checkDir("../Figure")
    checkDir("../Figure/Derma")
    for alpha in [0.1]:
        n, auxn, w = 200, 500, 0.03
        auxnumList = [1,2,3,4,5]
        RpathList = [f"../Result/Real_Derma/{n}_{auxn}_{auxnum}_{w}" for auxnum in auxnumList]
        nameList = ["LCP", "FCP", "FedCP", "FedCP-QQ", "CPlab", "CPhet"]
        ReList, _ = summation_matrix(RpathList, nameList, alpha)
        title = f'DERMA ($n={n},\\alpha={alpha}$)'
        savepath = f"../Figure/Derma/nagent_{n}_{alpha}.pdf"
        draw(ReList, title, auxnumList, savepath)

    Rpath = "../Result/Real_Derma/200_500_4_0.03"
    nameList = ["LCP", "FCP", "FedCP", "FedCP-QQ"]
    alphaList = [0.1]
    LoccovList, LoccovStdList = summation_loc([Rpath], nameList)
    savepath = f"../Figure/Derma/conditional.pdf"
