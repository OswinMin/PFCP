...
def predict_sum(fcp, predMap, test_agent_, test_agent, km, name, alpha, isLog, Lpath, log):
    indX = km.predict(predMap.covx(test_agent_.getX()))
...
if __name__ == '__main__':
    ...
    if len(sys.argv) > 1:
        n, auxnum, sepN, isLog = int(sys.argv[1]), int(sys.argv[2]), int(sys.argv[3]), True
    else:
        n, auxnum, sepN, isLog = 250, 4, 10, False
    ...
    typeNum = 7
    tar_w = [0.15,0.25,0.2,0.05,0.2,0.1,0.05]
    ...
    epoches, repeats, testN, d = 200, 100, np.sum(tar_N), 32
    hidden_dims_pred = [30, 30]
    hidden_dims_dre = [30]
    num_layer, hidden_dim, noise_dim = 2, 100, 32
    alpha = 0.1
    ...
    km = splitX(trainerMap.covx(agent_target.getX()), sepN)

    Rpath = f"../Result/Real_Derma/{n}_{auxnum}_{sepN}"
    Lpath = f"../Log/Real_Derma/{n}_{auxnum}_{sepN}.txt"
    checkDir("../Result")
    ...
    if isLog:
        relog(Lpath)
        dic = {'n': n, 'auxnum': auxnum, 'sepN':sepN, 'tar_N':tar_N, 'aux_N':aux_N, 'alpha': alpha, 'epoches': epoches, 'repeats': repeats, 'testN': testN, 'hidden_dims_pred': hidden_dims_pred, 'hidden_dims_dre': hidden_dims_dre, "num_layer": num_layer, "hidden_dim": hidden_dim, "noise_dim": noise_dim}
        logInfo(dic, Lpath)
    ...
    for rep in range(repeats):
        ...
        for i in range(len(predictorList)):
            ...
            predictorList[i].run_training(mute=True)
        ...
        fcdre.trainOnXS(engTrAgent[0], engTrAgent[1:], epoches=epoches, mute=True, caliN=0.1)
        fcdre_xy = FederatedCalibratedDRE(d, hidden_dims_dre, lr=5e-3, onXS=True, calibrate=True)
        fcdre_xy.trainOnXS(engTrAgent_xy[0], engTrAgent_xy, epoches=epoches // 10, mute=True, caliN=0.1)
        fcdre_y = FederatedCalibratedDRE(0, hidden_dims_dre, lr=5e-3, onXS=True, calibrate=True)
        fcdre_y.trainOnXS(engTrAgent_y[0], engTrAgent_y, epoches=epoches // 10, mute=True, caliN=0.1)
        ...
        setseed(seed_rep)
        CS0[rep, :, :], COV0[rep, :], SIZE0[rep, :], IND[rep, :] = predict_sum(fcp0, trainerMap, agent_test_, agent_test, km, 'LCP', alpha, isLog, Lpath, log)
        CS1[rep, :, :], COV1[rep, :], SIZE1[rep, :], _ = predict_sum(fcp1, trainerMap, agent_test_, agent_test, km, 'FCP', alpha, isLog, Lpath, log)
        CS2[rep, :, :], COV2[rep, :], SIZE2[rep, :], _ = predict_sum(fedcp, trainerMap, agent_test_, agent_test, km, 'FedCP', alpha, isLog, Lpath, log)
        CS3[rep, :, :], COV3[rep, :], SIZE3[rep, :], _ = predict_sum(fedcpqq, trainerMap, agent_test_, agent_test, km, 'FedCP-QQ', alpha, isLog, Lpath, log)
        CS4[rep, :, :], COV4[rep, :], SIZE4[rep, :], _ = predict_sum(cphet, trainerMap, agent_test_, agent_test, km, 'CPhet', alpha, isLog, Lpath, log)
        CS5[rep, :, :], COV5[rep, :], SIZE5[rep, :], _ = predict_sum(cplab, trainerMap, agent_test_, agent_test, km, 'CPlab', alpha, isLog, Lpath, log)
        tn = datetime.datetime.now().strftime('%H:%M:%S')
