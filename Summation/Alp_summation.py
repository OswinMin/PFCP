...
if __name__ == '__main__':
    ...
    for s, k in zip(['abs', 'cos'], [1, 2]):
        RpathList = [f"../Result/Alp_{s}/{d}_{n}_{agentN}_{gammaSelectRule}_{g}_{alpha}" for alpha in alphaList]
        ReList = [sum_path(rp, alpha) for rp, alpha in zip(RpathList, alphaList)]
        for name, j in zip(['GLCP','PFCP'], [0,1]):
            ...
            axes[k-1, 0].plot(covList, mar, color=col[j], linestyle=linestyle[j], marker=markers[j], linewidth=3, markersize=8, label=name)
            axes[k - 1, 0].errorbar(covList, mar, yerr=mar_std, color=col[j], alpha=0.9, linestyle='none')
            axes[k - 1, 0].set_title(f"S{k}.6")
            axes[k - 1, 0].set_ylabel(f"Marginal Coverage")
            axes[k-1, 1].plot(covList, con, color=col[j], linestyle=linestyle[j], marker=markers[j], linewidth=3, markersize=8, label=name)
            axes[k - 1, 1].set_title(f"S{k}.6")
            axes[k - 1, 1].set_ylabel(f"Miscoverage")
            axes[k-1, 2].plot(covList, size, color=col[j], linestyle=linestyle[j], marker=markers[j], linewidth=3, markersize=8, label=name)
            axes[k - 1, 2].errorbar(covList, size, yerr=size_std, color=col[j], alpha=0.9, linestyle='none')
            axes[k - 1, 2].set_title(f"S{k}.6")
            axes[k - 1, 2].set_ylabel(f"Size")
        axes[k - 1,0].set_xlabel("$1-\\alpha$")
        axes[k - 1,1].set_xlabel("$1-\\alpha$")
        axes[k - 1,2].set_xlabel("$1-\\alpha$")
    ...
    plt.savefig(f"../Figure/Alp/mar_cond.pdf", format='pdf')
    plt.show()