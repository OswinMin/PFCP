...
def draw(ReList:list[pd.DataFrame], title, auxInd, savepath):
    ...
    axes[1].set_ylim((min(np.min(FCP_size), np.max(FCP_size) / 6 * 5), axes[1].get_ylim()[1]))
...
if __name__ == '__main__':
    checkDir("../Figure")
    checkDir("../Figure/Achieve")
    for alpha in [0.1]:
        n, features, targ_g, targ_ind, sepN, seed = 100, 20, 3, 2, 20, 1
        auxInd = [1,2,3,4,5,6,7,8]
        RpathList = [f"../Result/Real_Achieve/{n}_{auxN}_{features}_{targ_g}_{targ_ind}_{sepN}_{seed}_{alpha}" for auxN in auxInd]
        nameList = ["LCP", "FCP", "FedCP", "FedCP-QQ", "CPlab", "CPhet"]
        ReList, _ = summation_matrix(RpathList, nameList, alpha)
        title = f'STAR ($n={n},\\alpha={alpha}$)'
        savepath = f"../Figure/Achieve/nagent_{n}_{alpha}.pdf"
        draw(ReList, title, auxInd, savepath)

    Rpath = "../Result/Real_Achieve/100_8_20_3_2_20_1_"
    nameList = ["LCP", "FCP", "FedCP", "FedCP-QQ"]
    alphaList = [0.1]
    LoccovList, LoccovStdList = summation_loc([Rpath+f"{a}" for a in alphaList], nameList)
    savepath = f"../Figure/Achieve/conditional.pdf"
