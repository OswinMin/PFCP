...
if __name__ == '__main__':
    ...
    engTrAgent_ = [Agent(a.X, a.Y) for a in engTrAgent]
    for a in engTrAgent_:
        a.loadS(a.Y)
    fcdre = FederatedCalibratedDRE(32, [10], lr=5e-3, onXS=True, calibrate=False)
    fcdre.trainOnXS(engTrAgent_[0], engTrAgent_[1:], caliN=5, epoches=100, log=log, mute=False)

    setseed(0)
    cphetcl = CPhetCl(trainer[0], engTrAgent[0:], fcdre, .1)
    cs = cphetcl.predict(X)
    size = cs.sum(-1).mean()
