...
if __name__ == '__main__':
    checkDir("../Figure")
    checkDir("../Figure/Crime")
    for alpha in [0.1]:
        n, features, sepN, seed = 75, 20, 20, 1
        auxInd = [1,2,3,4,5,6,7]
        RpathList = [f"../Result/Real_Crime/{n}_{features}_{sepN}_{seed}_{auxN}_{alpha}" for auxN in auxInd]
        nameList = ["LCP", "FCP", "FedCP", "FedCP-QQ", "CPlab", "CPhet"]
        ReList, _ = summation_matrix(RpathList, nameList, alpha)
        title = f'CRIME ($n={n},\\alpha={alpha}$)'
        savepath = f"../Figure/Crime/nagent_{n}_{alpha}.pdf"
        draw(ReList, title, auxInd, savepath)

    Rpath = "../Result/Real_Crime/100_20_20_1_4_"
    nameList = ["LCP", "FCP", "FedCP", "FedCP-QQ"]
    alphaList = [0.1]
    LoccovList, LoccovStdList = summation_loc([Rpath+f"{a}" for a in alphaList], nameList)
    savepath = f"../Figure/Crime/conditional.pdf"
