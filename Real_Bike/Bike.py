...
def selectTarget(X, y, split_index, season, yr, n):
    mask = (split_index['season'] == season) & (split_index['yr'] == yr)
    agent_target = Agent(X[mask], y[mask])
    X, y, split_index = X[~mask], y[~mask], split_index[~mask]
    unique_index = np.unique(split_index[['yr','mnth']], axis=0)
    agent_aux = []
    for i in range(len(unique_index)):
        mask = (split_index['yr'] == unique_index[i,0]) & (split_index['mnth'] == unique_index[i,1])
        if mask.sum() >= 4*n:
            agent_aux.append(Agent(X[mask], y[mask]))
...
if __name__ == '__main__':
    """
    Split agent using season yr mnth
    Target: 8 choices (1 season in a year)
    Rest monthly data serve as auxiliary agent
    hr used to split the support of X to calculate conditional coverage
    """
    if os.path.split(os.getcwd())[1] != 'Real_Bike':
        os.chdir(os.path.join(os.getcwd(), 'Real_Bike'))
    bikedata = pd.read_csv("../Dataset/bikeSharing/hour.csv")
    split_index = bikedata[['season', 'yr', 'mnth']]
    X_raw = bikedata.drop(['season', 'yr', 'mnth', 'instant', 'dteday', 'casual', 'registered', 'cnt'], axis=1)
    scaler = StandardScaler()
    X = pd.DataFrame(scaler.fit_transform(X_raw), columns=X_raw.columns).values
    y = np.log(bikedata['cnt'].values+1)
    d = X_raw.shape[1]

    # each agent has 3*n samples
    # auxType = 0 : pred tr : eng tr = 1 : 2, otherwise 1 : 1
    n, season, yr, seed, sepN, auxN, alpha = 100, 4, 1, 1, 20, 20, .2
    if len(sys.argv) > 1:
        n, season, yr, seed, sepN, auxN, alpha = int(sys.argv[1]), int(sys.argv[2]), int(sys.argv[3]), int(sys.argv[4]), int(sys.argv[5]), int(sys.argv[6]), float(sys.argv[7])
        isLog = True
    else:
        isLog = False

    Rpath = f"../Result/Real_Bike/{n}_{yr}_{season}_{seed}_{auxN}_{alpha}"
    Lpath = f"../Log/Real_Bike/{n}_{yr}_{season}_{seed}_{auxN}_{alpha}.txt"
    checkDir("../Result")
    checkDir("../Result/Real_Bike")
    checkDir(Rpath)
    checkDir("../Log")
    checkDir("../Log/Real_Bike")

    setseed(seed)
    agent_target, agent_aux = selectTarget(X, y, split_index, season, yr, n)
    agent_aux = agent_aux[-auxN:]
    epoches, repeats, testN = 300, 100, min(agent_target.n - 3*n,500)
    hidden_dims_pred = [30, 30]
    hidden_dims_dre = [30]
    num_layer, hidden_dim, noise_dim = 2, 100, d
    km = splitX(agent_target.getX(), sepN)

    if isLog:
        relog(Lpath)
        dic = {'d': d, 'n': n, 'season': season, 'yr': yr, 'seed': seed, 'sepN':sepN, 'auxN':auxN, 'alpha': alpha, 'epoches': epoches, 'repeats': repeats, 'testN': testN, 'hidden_dims_pred': hidden_dims_pred, 'hidden_dims_dre': hidden_dims_dre, "num_layer": num_layer, "hidden_dim": hidden_dim, "noise_dim": noise_dim}
        logInfo(dic, Lpath)
    log(f"Target data pool with {agent_target.n} samples, auxiliary agent with {' '.join([f'{a.n}' for a in agent_aux])} samples. In total {len(agent_aux)} auxiliary agents. Test sample size is {testN}", path=Lpath, islog=isLog)
    ...
    for rep in range(repeats):
        ...
        predTrAgent, engTrAgent = sampleAux(agent_aux, n)
