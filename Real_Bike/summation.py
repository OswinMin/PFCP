...
if __name__ == '__main__':
    checkDir("../Figure")
    checkDir("../Figure/Bike")
    for alpha in [0.1]:
        n, season, yr, seed, sepN = 100, 4, 1, 1, 20
        auxInd = [6,8,10,12,14,16,18,20]
        RpathList = [f"../Result/Real_Bike/{n}_{yr}_{season}_{seed}_{auxN}_{alpha}" for auxN in auxInd]
        nameList = ["LCP", "FCP", "FedCP", "FedCP-QQ", "CPlab", "CPhet"]
        ReList, _ = summation_matrix(RpathList, nameList, alpha)
        title = f'BIKE ($n={n},\\alpha={alpha}$)'
        savepath = f"../Figure/Bike/nagent_{n}_{alpha}.pdf"
        draw(ReList, title, auxInd, savepath)

    Rpath = "../Result/Real_Bike/100_1_4_1_12_"
    nameList = ["LCP", "FCP", "FedCP", "FedCP-QQ"]
    alphaList = [0.1]
    LoccovList, LoccovStdList = summation_loc([Rpath+f"{a}" for a in alphaList], nameList)
    savepath = f"../Figure/Bike/conditional.pdf"
