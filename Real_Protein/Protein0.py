...
def summation_real(cov, size, ind, X_int, name, alpha=.1, isLog=False, path='', log=log):
    mar = np.mean(cov)
    mean_size = np.mean(size)
    loc_cov = np.zeros(X_int)
    weight = np.zeros(X_int)
    for i in range(X_int):
        loc_cov[i] = np.mean(cov[ind == i])
        weight[i] = np.mean(ind == i)
...
def sampleAux(X, y, y_loc, y_scale, y_index, n):
    """
    根据loc,scale划分pred agent和eng agent，前者每个n个样本，后者2n个
    """
    predTrAgent_list = []
    engTrAgent_list = []
    for y_ind in y_index:
        ind = selectN(y, loc=y_loc[y_ind], scale=y_scale[y_ind], n=3*n)
        X_, y_, X, y = splitData(X, y, ind)
        agent_ = Agent(X_, y_)
        predTrAgent, engTrAgent = agent_.splitAgent(agent_.n//2)
...
if __name__ == '__main__':
    if os.path.split(os.getcwd())[1] != 'Real_Protein':
        os.chdir(os.path.join(os.getcwd(), 'Real_Protein'))
    protdata = pd.read_csv("../Dataset/proteinStructure.csv")
    X_raw = protdata.drop('RMSD', axis=1)
    y = np.log(protdata['RMSD'].values+1)
    d = X_raw.shape[1]

    # each agent has 3*n samples
    n, agentN, tar_ind, seed, X_int, alpha = 100, 20, 9, 42, 30, .1
    if len(sys.argv) > 1:
        n, agentN, tar_ind, seed, X_int, alpha = int(sys.argv[1]), int(sys.argv[2]), int(sys.argv[3]), int(sys.argv[4]), int(sys.argv[5]), float(sys.argv[6])
        isLog = True
    else:
        isLog = False
    epoches, repeats, testN = 500, 100, 3000
    hidden_dims_pred = [30, 30]
    hidden_dims_dre = [30]
    num_layer, hidden_dim, noise_dim = 2, 100, d
    Y_int = 10

    Rpath = f"../Result/Real_Protein/{n}_{agentN}_{tar_ind}_{X_int}_{seed}_{alpha}"
    Lpath = f"../Log/Real_Protein/{n}_{agentN}_{tar_ind}_{X_int}_{seed}_{alpha}.txt"
    checkDir("../Result")
    checkDir("../Result/Real_Protein")
    checkDir(Rpath)
    checkDir("../Log")
    checkDir("../Log/Real_Protein")

    setseed(seed)
    X_transformed, skewed_cols = auto_skew_transform(X_raw.copy(), log, Lpath, isLog, skew_threshold=1.0)
    ...
    y_scale = [(y_loc[i+1]-y_loc[i-1])/2 for i in range(1,10)]
    y_scale = [y_scale[0]] + y_scale + [y_scale[-1]]

    # 用 F1-9 作为划分X的依据计算条件coverage，反映空间结构和二级结构限制
    Xt, yt, X, y = splitData(X, y, selectN(y, y_loc[tar_ind], y_scale[tar_ind], testN+3*n))
    agent_target = Agent(Xt, yt)
    km = splitX(Xt, X_int)
    log(f"Target data pool with {Xt.shape[0]} samples, left {X.shape[0]} samples.", path=Lpath, islog=isLog)

    if isLog:
        relog(Lpath)
        dic = {'d':d, 'n':n, 'agentN':agentN, 'target_Y_ind':tar_ind, 'seed':seed, 'alpha':alpha, 'epoches':epoches, 'repeats':repeats, 'testN':testN, 'hidden_dims_pred':hidden_dims_pred, 'hidden_dims_dre':hidden_dims_dre, "num_layer":num_layer, "hidden_dim":hidden_dim, "noise_dim":noise_dim, "X_interval":X_int, "Y_interval":Y_int}
        logInfo(dic, Lpath)
    ...
    for rep in range(repeats):
        ...
        y_index = np.random.choice(range(1,10), agentN)
        predTrAgent, engTrAgent = sampleAux(X, y, y_loc, y_scale, y_index, n)
    ...
    CS0, COV0, SIZE0 = loadData(Rpath, "LCP")
    CS1, COV1, SIZE1 = loadData(Rpath, "FCP")
    CS2, COV2, SIZE2 = loadData(Rpath, "FCP_")
    CS3, COV3, SIZE3 = loadData(Rpath, "FedCP")
    CS4, COV4, SIZE4 = loadData(Rpath, "FedCP-QQ")
    CS5, COV5, SIZE5 = loadData(Rpath, "CPhet")
    CS6, COV6, SIZE6 = loadData(Rpath, "CPlab")
    summation_real(COV0, SIZE0, IND, X_int, "LCP", alpha, isLog, Lpath, log)
    summation_real(COV1, SIZE1, IND, X_int, "FCP", alpha, isLog, Lpath, log)
    summation_real(COV2, SIZE2, IND, X_int, "FCP_", alpha, isLog, Lpath, log)
    summation_real(COV3, SIZE3, IND, X_int, "FedCP", alpha, isLog, Lpath, log)
    summation_real(COV4, SIZE4, IND, X_int, "FedCP-QQ", alpha, isLog, Lpath, log)
    summation_real(COV5, SIZE5, IND, X_int, "CPhet", alpha, isLog, Lpath, log)
    summation_real(COV6, SIZE6, IND, X_int, "CPlab", alpha, isLog, Lpath, log)